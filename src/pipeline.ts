...
export async function runPlanningPhase(
	...
	const { result: planRaw, provider: planProvider } = await walkProviderChain(env, providers, (p, apiKey) => {
		const model = p.planModel ?? p.generateModel;
		const maxTokens = p.planModel ? PLAN_THINKING_MAX_TOKENS : PLAN_MAX_TOKENS;
	...
	console.log(`Plan generated by provider "${planProvider.name}" (model=${planProvider.planModel ?? planProvider.generateModel})`);
